...
如果文章内容参考或提炼自其他来源（推文、博客、论文等），必须在文章末尾添加"参考来源"区块：
...
- [来源标题](URL) — 一句话说明引用了什么内容
- [来源标题](URL) — 一句话说明
```

- 用 `---` 分隔线与正文隔开
- 每条来源附简短说明，说清楚引用了什么（框架、数据、观点等）
- 用户提供的原始链接必须保留，不要省略
- 即使是自己的理解和重写，也要标注灵感来源，尊重原作者
...
## 进阶优化规则（仅当用户要求"优化"时应用）

以下规则不在默认写作流程中使用，仅当用户明确要求优化文章质量时才应用：

### 结构优化
- 开头 2-3 段概述全文，不寒暄直接进主题
- 先给直觉类比（让读者 3 秒建立认知框架），再展开细节
- 每个大章节遵循"变化是什么 → 为什么变 → 怎么做"的逻辑
- 章节之间用类比句做过渡，不要硬切
